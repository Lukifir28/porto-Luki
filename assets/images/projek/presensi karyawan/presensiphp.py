...
try:
    db = sql.connect(host=HOST, user=USER, password=PASS)
    cursor = db.cursor()
    print('>> Koneksi berhasil')
    try:
        cursor.execute('''CREATE DATABASE IF NOT EXISTS {0}'''.format(DATB))
...
def simpan ():
    ...
    try:
        db1 = sql.connect(host=HOST, user=USER, password=PASS, database=DATB)
        cursor1 = db1.cursor()
        print('>> Terhubung ke database')
        try:
            cursor1.execute('''CREATE TABLE IF NOT EXISTS presensi (ID INT (45) NOT NULL AUTO_INCREMENT PRIMARY KEY, Nama TEXT, Jabatan TEXT, Foto LONGBLOB)''')
            print('>> Tabel siap digunakan')
            try:
                with open(FILE, 'rb') as File:
                    data_biner = File.read()
                perintahSQL1 = 'INSERT INTO presensi (Nama, Jabatan, Foto) VALUES (%s, %s, %s)'
